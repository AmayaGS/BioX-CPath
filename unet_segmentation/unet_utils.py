...
def create_mask_and_patches(loader, model, batch_size, mean, std, device, path_to_save_mask_and_df, path_to_save_patches, coverage, keep_patches, patient_id_parsing):
    ...
    filename = os.path.join(path_to_save_mask_and_df, "extracted_patches.csv")
    ...
    with torch.no_grad():
        ...
        for batch_idx, (img_patches, img_indices, label) in enumerate(loop):
            ...
            loop.set_description(f"Processing WSI: {name}, with {num_patches} patches")
                ...
                if keep_patches:
                    for patch in range(len(pred_patch_array)):
                        ...
                        if (white_pixels / len(pred_patch_array[patch])**2) > coverage:
                            ...
                            file_location = os.path.join(folder_location, patch_name)
            ...
            binary_mask_path = os.path.join(path_to_save_mask_and_df, "binary_mask/")
            os.makedirs(binary_mask_path, exist_ok=True)
            plt.imsave(os.path.join(binary_mask_path, name + ".png"), merged_pre)
