...
from embedding_utils import create_non_stratified_split, seed_everything, collate_fn_none, create_stratified_splits, create_embedding_graphs
...
def main(args):
    ...
    output_directory = os.path.join(args.directory, "features")
    ...
    if not os.path.exists(sss_dict_name):
        if (args.dataset_name.upper() not in ["CAMELYON16", "CAMELYON17", "CAMELYON"]):
            ...
            create_non_stratified_split(extracted_patches, patient_labels, args.patient_id, args.label, args.dataset_name)
    ...
    with open(output_directory + f"/embedding_dict_{args.dataset_name}_{args.embedding_net}.pkl", "wb") as file:
        pickle.dump(embedding_dict, file)  # encode dict into Pickle
        print("Done writing embedding_dict into pickle file")

    with open(output_directory + f"/knn_dict_{args.dataset_name}_{args.embedding_net}.pkl", "wb") as file:
        pickle.dump(knn_dict, file)  # encode dict into Pickle
        print("Done writing knn_dict into pickle file")

    with open(output_directory + f"/rag_dict_{args.dataset_name}_{args.embedding_net}.pkl", "wb") as file:
        pickle.dump(rag_dict, file)  # encode dict into Pickle
        print("Done writing rag_dict into pickle file")

    with open(output_directory + f"/krag_dict_{args.dataset_name}_{args.embedding_net}.pkl", "wb") as file:
