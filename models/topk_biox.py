...
class SelectTopK(Select):
    ...
    def forward(
    ...
    ) -> tuple[SelectOutput, SelectOutput]:
        """"""  # noqa: D419
        if batch is None:
        ...
        if self.min_score is None:
            normalized_score = score / self.weight.norm(p=2, dim=-1)
            # use softsign instead of tanh, it has a range of (-1, 1), is symmetric around 0,
            # but with polynomic slope instead of exponential helping to avoid ties in -1/1
            score = F.softsign(normalized_score)
            #score = self.act(score / self.weight.norm(p=2, dim=-1), )
            score = score + torch.randn_like(score) * 1e-6 # Add random noise to break any potential remaining ties
        else:
            normalized_score = score / self.weight.norm(p=2, dim=-1)
        ...
        top_k_atten_weights = SelectOutput(
        ...
        top_k_node_weights = SelectOutput(
            node_index=node_index,
            num_nodes=x.size(0),
            cluster_index=torch.arange(node_index.size(0), device=x.device),
            num_clusters=node_index.size(0),
            weight=normalized_score[node_index])

        return top_k_atten_weights, top_k_node_weights
