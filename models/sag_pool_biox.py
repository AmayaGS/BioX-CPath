...
from models.topk_biox import SelectTopK
...
class SAGPooling(torch.nn.Module):
    ...
    def forward(
        ...
        select_out, select_out_nodes = self.select(attn, batch)

        perm = select_out.node_index
        score = select_out.weight
        node_score = select_out_nodes.weight
        assert score is not None

        x = x[perm] * score.view(-1, 1) # weight the output features with the attention scores
        ...
        return (x, connect_out.edge_index, connect_out.edge_attr,
                connect_out.batch, perm, score, node_score)
